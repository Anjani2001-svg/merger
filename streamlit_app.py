...
if not ONEDRIVE_AVAILABLE:
    ...
    if token:
        ...
        if st.button("🔍 Test Folder Connection", type="secondary"):
            ...
            # verify token
            me = _req.get("https://graph.microsoft.com/v1.0/me", headers=_h, timeout=15)
            if me.status_code != 200:
                st.error(f"❌ Token invalid. Sign out and reconnect.")
            else:
                st.success(f"✅ Signed in as **{me.json().get('displayName','?')}**")

                # personal OneDrive search
                r1 = _req.get(
                    f"https://graph.microsoft.com/v1.0/me/drive/root/search"
                ...
                if hits:
                    ...
                    st.warning("⚠️ Not in personal OneDrive — checking shared items…")

                    # Try ALL possible sharedWithMe endpoints
                    all_shared = []
                    for endpoint in [
                        "https://graph.microsoft.com/v1.0/me/drive/sharedWithMe?$top=999",
                        "https://graph.microsoft.com/v1.0/me/drive/sharedWithMe?$select=id,name,folder,remoteItem,parentReference&$top=999",
                    ]:
                        r2 = _req.get(endpoint, headers=_h, timeout=20)
                        if r2.status_code == 200:
                            items = r2.json().get("value", [])
                            for it in items:
                                if it not in all_shared:
                                    all_shared.append(it)
                            # follow next pages
                            nxt = r2.json().get("@odata.nextLink")
                            while nxt:
                                rn = _req.get(nxt, headers=_h, timeout=20)
                                if rn.status_code != 200: break
                                all_shared.extend(rn.json().get("value", []))
                                nxt = rn.json().get("@odata.nextLink")

                    # Also try /me/drive/root/children for top-level shared
                    r3 = _req.get(
                        "https://graph.microsoft.com/v1.0/me/drive/root/children"
                        "?$select=id,name,folder,shared&$top=999",
                        headers=_h, timeout=15)
                    if r3.status_code == 200:
                        for it in r3.json().get("value", []):
                            if "shared" in it and it not in all_shared:
                                all_shared.append(it)

                    st.write(f"**Total shared/visible folders:** {len(all_shared)}")
                    all_names = sorted([i.get("name","?") for i in all_shared])
                    st.write("**All items:**", all_names)

                    matches = [i for i in all_shared
                               if onedrive_folder.lower() in i.get("name","").lower()]
                    if matches:
                        m = matches[0]
                        remote = m.get("remoteItem", {})
                        drv = (remote.get("parentReference",{}).get("driveId","")
                               or m.get("parentReference",{}).get("driveId",""))
                        fid = remote.get("id","") or m.get("id","")
                        st.success(f"✅ Found: `{m.get('name')}` driveId=`{drv[:20]}…` id=`{fid[:20]}…`")
                        st.info("Upload should work. Try clicking ☁ Upload to OneDrive after merging.")
                    else:
                        st.error(
                            f"❌ **'{onedrive_folder}'** not found.\n\n"
                            f"**Options:**\n"
                            f"1. Paste the folder URL in the field below\n"
                            f"2. Ask Sadeev to share directly to your email via OneDrive (not Teams)\n"
                            f"3. Copy the exact folder name from the list above")
    else:
        st.markdown(
